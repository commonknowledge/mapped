...
import { Combobox } from './ui/ConsComboBox'
    ...
    <div className="flex flex-col">
      ...
      {constituencies?.length === 0 ? (
        <div className="text-left p-4 text-meepGray-300 flex items-center gap-2 bg-meepGray-700 rounded-lg">
          ...
          <p>
            Add data to the <span className=' bg-meepGray-500 px-2 py-1 rounded-sm'><Layers className="inline-block w-4 mr-1" />Map Layers</span> panel first to see constituency data
          </p>
        </div>
      ) : (
        <div className="max-h-[70vh] overflow-y-auto ">
          {constituencies
            ?.filter(constituency => !selectedConstituency || constituency.gss === selectedConstituency)
            ?.map((constituency) => (
              <div
